...
export class initDatabase1629004156222 implements MigrationInterface {
    name = 'initDatabase1629004156222'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`CREATE TABLE \`my_app\`.\`users\` (\`id\` int NOT NULL AUTO_INCREMENT, \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`deleted_at\` datetime(6) NULL, \`created_by_id\` int NULL, \`updated_by_id\` int NULL, \`deleted_by_id\` int NULL, \`email\` varchar(255) NOT NULL, \`name\` varchar(255) NOT NULL, \`password\` varchar(255) NOT NULL, \`avatar\` varchar(255) NULL, \`permissions\` text NOT NULL, \`status\` enum ('pending', 'active', 'blocked') NOT NULL DEFAULT 'pending', UNIQUE INDEX \`IDX_97672ac88f789774dd47f7c8be\` (\`email\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`my_app\`.\`user_token\` (\`user_id\` int NOT NULL, \`iat\` int NOT NULL, \`exp\` int NOT NULL, PRIMARY KEY (\`user_id\`, \`iat\`)) ENGINE=InnoDB`);
